    ...
    private void getresponse() {
        ...
        call.enqueue(new Callback<List<response>>() {
            ...
            public void onResponse(Call<List<response>> call, Response<List<response>> response) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    if (list.get(i).getMarket().substring(list.get(i).getMarket().length() - 3).equalsIgnoreCase("BTC")) {
                        responseList.add(list.get(i));
                    }
                }
                recyclerView.setAdapter(new bAdapter(view.getContext(), responseList, map, pair, 3));
            }
