...
public class usdtFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        view = inflater.inflate(R.layout.fragment_usdt, container, false);
    ...
    private void getresponse() {
        ...
        call.enqueue(new Callback<List<response>>() {
            ...
            public void onResponse(Call<List<response>> call, Response<List<response>> response) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    if (list.get(i).getMarket().substring(list.get(i).getMarket().length() - 4).equalsIgnoreCase("USDT")) {
                        responseList.add(list.get(i));
                    }
                }
                recyclerView.setAdapter(new uAdapter(view.getContext(), responseList, map, pair, 2));
            }
